...
plugin:addHook("AccountTicketFound", function(acc)
	...
	if playerCount >= maxPublicSlots or ServerOpen then
		if acc then
			...
			hook.once("SendConnectResponse", function(_, _, data)
				if maxPublicSlots == 0 then
					data.message = "The server is currently closed! Check the discord for more info."
					print(
						string.format(
							"%s (%s) was denied access to the server",
							acc.name,
