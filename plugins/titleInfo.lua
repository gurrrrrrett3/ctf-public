...
local TPS = memory.getBaseAddress() + 0xD1D82
local memory_readUByte = memory.readUByte
...
plugin:addHook("Logic", function()
	...
	if infoCounter == infoInterval then
		...
		-- server:setConsoleTitle(
		-- 	string.format(
		-- 		"%s | %i player%s | %.2f TPS (%.2f%%)",
		-- 		server.name,
		-- 		numPlayers,
		-- 		numPlayers == 1 and "" or "s",
		-- 		recentOne,
		-- 		recentOne / 62.5 * 100
		-- 	)
		-- )

		if memory_readUByte(TPS) == 15 and recentFiveSec < 50 then
...
plugin.commands["/tps"] = {
	...
	call = function(ply)
		ply:sendMessage(
			string.format(
				"TPS from last 5s, 1m, 5m, 15m: %.2f, %.2f, %.2f, %.2f",
				recentFiveSec,
				recentOne,
				recentFive,
				recentFifteen
