...
---Convert eulger angles to a RotMatrix.
---@param pitch number The pitch (X) angle (in radians).
---@param yaw number The yaw (Y) angle (in radians).
...
---@param rot RotMatrix The rotation matrix to convert.
---@return number pitch The pitch (X) angle (in radians).
...
--Create a virtual copy of an object.
---@param object table The object to copy.
---@param destination? table The table to copy the object to.
---@return table copy The copied object.
function createVirtualTable(object, destination)
	destination = destination or {}
	for key, value in pairs(object) do
		if type(value) == "table" then
			destination[key] = createVirtualTable(value)
		else
...
function SetTPSDelay(ms)
	memory.writeByte(memory.getBaseAddress() + 0xD1D82, ms)
end
...
function decimalToHex(decimal)
	local hexChars = "0123456789abcedef"
	local hexResult = ""
